<template>
    ...
    <div v-else class="border rounded pb-3 px-2">
      ...
      >
        <div>
          <h1 class="products-heading-text">Products</h1>
          <p class="mb-0 text-muted small">Manage your product listings</p>
        </div>
        <router-link :to="{name:APP_ROUTE_NAMES.PRODUCT_CREATE}" class="btn btn-orange btn-sm gap-2 rounded-1 px-4 py-2">
            ...
            <tbody>
              <tr v-for="product in products" :key="product.id">
                ...
                <td class="pe-3 text-end">
                  <button @click="router.push({name:APP_ROUTE_NAMES.PRODUCT_UPDATE,params:{id:product.id}})" class="btn btn-sm btn-outline-secondary m-2">
...
const handleProductDelete=async(productId)=>{
  try{
    loading.value=true;
    const confirmResult=await showConfirm('Are you sure you want to delete the product?');
